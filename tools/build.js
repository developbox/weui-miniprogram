...
function buildLess(lessFileList) {
    ...
    return (
        gulp
            .src(lessFileList, { cwd: srcPath, base: srcPath })
            .pipe(
...
function install() {
    return gulp.series(
        ...
        () => {
            ...
            return gulp.src(demoPackageJsonPath).pipe(gulpInstall({ production: true }))
        }
    )
}
    ...
    init() {
        ...
        gulp.task(`${id}-component-json`, (done) => {
            ...
            if (jsonFileList && jsonFileList.length) {
                // 去掉 index.json
                return copy(
                    this.componentListMap.jsonFileList.filter((file) => file !== 'index.json')
                )
            }
        ...
        gulp.task(`${id}-watch-demo`, () => {
            ...
            const watchCallback = (filePath) =>
                gulp.src(filePath, { cwd: demoSrc, base: demoSrc }).pipe(gulp.dest(demoDist))
